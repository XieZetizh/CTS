...
local function ProcessChassis(chassis)
    if not chassis or chassis.Name == "Chassis" .. LocalPlayer.Name then return end
    
    local hull = chassis:FindFirstChild("Hull")
    if hull and not ESP.Instances[hull] then
    ...
    if turret and not ESP.Instances[turret] then
        local hl = Instance.new("Highlight")
        hl.Adornee = turret
        hl.Parent = ESPFolder
        hl.FillColor = ESP.TurretColor
        
        ESP.Instances[turret] = {
            Instance = hl,
            Color = ESP.TurretColor,
            IsHull = false,
            Model = chassis
        }
    end
